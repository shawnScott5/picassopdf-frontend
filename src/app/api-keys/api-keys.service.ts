...
export class apiKeysService {
  ...
  getApiKeys(userParams: { userId: string; organizationId?: string }): Observable<any> {
    return this.http.get<any>(`${this.apiConfig.apiUrl}/api-keys`, { params: userParams });
  }

  // Get all API keys (for dashboard filtering)
  getAllApiKeys(): Observable<any> {
    return this.http.get<any>(`${this.apiConfig.apiUrl}/api-keys`);
  }
  ...
  createApiKey(payload: {
    ...
    console.log(payload)
    return this.http.post<ApiResponse<any>>(`${this.apiConfig.apiUrl}/api-keys/create`, payload);
  }

  // Deactivate an API key
  deactivateApiKey(apiKeyId: string, userId: string): Observable<ApiResponse<any>> {
    console.log('DEACTIVATING API KEY..........', apiKeyId);
    return this.http.put<ApiResponse<any>>(`${this.apiConfig.apiUrl}/api-keys/deactivate/${apiKeyId}`, { userId });
  }

  // Activate an API key
  activateApiKey(apiKeyId: string, userId: string): Observable<ApiResponse<any>> {
    console.log('ACTIVATING API KEY..........', apiKeyId);
    return this.http.put<ApiResponse<any>>(`${this.apiConfig.apiUrl}/api-keys/activate/${apiKeyId}`, { userId });
  }

  // Delete an API key
  deleteApiKey(apiKeyId: string, userId: string): Observable<ApiResponse<any>> {
    console.log('DELETING API KEY..........', apiKeyId);
    return this.http.delete<ApiResponse<any>>(`${this.apiConfig.apiUrl}/api-keys/delete/${apiKeyId}`, { 
      body: { userId } 
    });
  }
  ...
  checkApiKeyNameAvailability(userId: string, name: string): Observable<any> {
    return this.http.get<any>(`${this.apiConfig.apiUrl}/api-keys/check-name`, { 
      params: { userId, name } 
    });
  }
  ...
  editApiKey(apiKeyId: string, payload: {
    ...
    return this.http.put<ApiResponse<any>>(`${this.apiConfig.apiUrl}/api-keys/edit/${apiKeyId}`, payload);
  }
  ...
  regenerateApiKey(apiKeyId: string, payload: {
    ...
    return this.http.put<ApiResponse<any>>(`${this.apiConfig.apiUrl}/api-keys/regenerate/${apiKeyId}`, payload);
  }
}
